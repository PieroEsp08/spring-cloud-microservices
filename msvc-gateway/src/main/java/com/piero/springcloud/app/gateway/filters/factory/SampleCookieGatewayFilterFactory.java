package com.piero.springcloud.app.gateway.filters.factory;
...
public class SampleCookieGatewayFilterFactory extends AbstractGatewayFilterFactory<SampleCookieGatewayFilterFactory.ConfigurationCookie> {
    ...
    public GatewayFilter apply(ConfigurationCookie config) {
        return new OrderedGatewayFilter((exchange, chain) ->{
            logger.info("ejecutando pre getway filter factory:"+ config.message);
            return chain.filter(exchange).then(Mono.fromRunnable(() ->{
                ...
                logger.info("ejecutando post getway filter factory:"+ config.message);
            }));
        },100);
    }
